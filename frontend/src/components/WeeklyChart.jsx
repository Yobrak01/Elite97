...
export const WeeklyChart = ({ data = [] }) => {
  const customTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className="glass-panel rounded-xl p-3 border border-white/10 text-xs shadow-glow-blue/20">
          <p className="font-bold text-white mb-1">{payload[0].payload.dayName || 'Date'}</p>
          <p className="text-accent-blue font-semibold">Hours: {payload[0].value} hrs</p>
          {payload[1] && <p className="text-cyan-400 font-semibold">Focus: {payload[1].value}%</p>}
        </div>
      );
    }
    return null;
