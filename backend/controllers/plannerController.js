...
exports.autoGenerateWeaknessTasks = async (user) => {
  ...
  for (const cu of courseUnits) {
    ...
    if (needsNewTask) {
      ...
      const aiData = computeAiTier(taskBase, cu.aiSuggestedTier);
      taskBase.aiSuggestedTier = aiData.tier;
      taskBase.tierScore = aiData.score;
      
      tasksToCreate.push(taskBase);
    }
  }
...
exports.getDailyPlan = async (req, res, next) => {
  try {
    ...
    const workout = await Workout.findOne({ user: req.user._id, date: today });
    const mealPlan = await MealPlan.findOne({ user: req.user._id, date: today });
    
    let planData;
    if (req.user.studyMode === 'cat_prep') {
      planData = await aiPlanner.generateGeminiCatPlan(tasks, courseUnits, req.user.settings, req.user.timetable, workout, mealPlan);
    } else {
      planData = await aiPlanner.generateDailyPlan(tasks, currentMode, req.user.settings, req.user.studyMode, req.user.timetable, workout, mealPlan);
    }
    ...
    for (let i = 0; i < planData.blocks.length; i++) {
      ...
      if (block.category === 'study' && (!block.taskId || block.taskId === "null" || block.taskId === "")) {
        ...
        let existingTask = await Task.findOne({
          ...
          description: '[Auto-Generated by AI Planner from Daily Schedule]',
          status: { $ne: 'completed' }
        });
        ...
        if (!existingTask) {
          ...
          let taskData = {
            ...
            description: '[Auto-Generated by AI Planner from Daily Schedule]',
            priority: 2,
            estimatedHours: block.duration ? (block.duration / 60) : 1.0,
            type: 'revision'
          };
          
          const tierResult = computeAiTier(taskData, null);
          taskData.aiSuggestedTier = tierResult.tier;
          taskData.tierScore = tierResult.score;

          existingTask = await Task.create(taskData);
        }
        planData.blocks[i].taskId = existingTask._id.toString();
      }
    }
